...
const Events = () => {
  ...
  return (
    <div className='w-full flex bg-white rounded-md flex-col gap-3 shadow-lg py-3 px-4'>
      ...
      <hr />
      {events.map((items)=>(
    <div key={items.id} className={`w-full  items-center justify-center flex cursor-pointer bg-${items.color} h-32 rounded-md`}>
    <div className=' flex flex-col items-center justify-center text-white'><span className=''>{items.icon}</span><span className='text-lg font-semibold'>{items.desc}</span></div>
