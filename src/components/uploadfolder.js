...
const Uploadfolder = ({close}) => {
  ...
  const handleOnchange = (event) => {
    ...
    if (!files || files.length === 0) {
      console.log("No files selected or unsupported folder upload.");
      return;
    }
    ...
    <div>
        ...
        <div className='w-1/2 text-center h-96 shadow-lg rounded-md bg-white flex flex-col px-6 py-6 gap-8'>
          ...
          <div className='flex flex-col gap-2 items-center justify-center'>
            ...
            <input onChange={handleOnchange}  type='file'  id="folderInput" webkitdirectory="true" directory="true" multiple className='flex items-center p-2 absolute border opacity-0 rounded-md bg-cta text-white'/>
            </label>
