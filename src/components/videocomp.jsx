...
const renderBackButton = ({ isDisabled, onClick }) => (
    <button
        className="absolute p-2 hover:bg-cta hover:text-white bg-gray-100 rounded-full -ml-[26rem] -mt-20"
        onClick={onClick}
        disabled={isDisabled}
    >
<Icon icon="ic:twotone-arrow-back-ios" /> </button>
  ...
  <div className='flex w-full flex-col font-semibold gap-6'>
        <div className='flex justify-between'> <span className='text-lg'>Videos ({videos.length})</span> <span onClick={seeAllVideos} className='cursor-pointer text-sm'>See All</span></div>
