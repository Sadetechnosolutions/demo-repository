...
const handleOption = ()=>{
  ...
  return (
    <div>
        <div className="max-w-[30rem] w-full gap-2 flex flex-col">
          {photo.postType === 'IMAGE' ? (
            <img className='w-full h-64' src={`http://192.168.1.4:8086${photo.imageUrl}`} alt='' />
          ) : photo.postType === 'VIDEO' ? (
            <video className='w-full h-96' controls>
              <source src={`http://192.168.1.4:8086${photo.videoUrl}`} type="video/mp4" />
          ...
          <div className="flex items-center justify-between">
                    ...
                    <img className="border w-9 h-9 rounded-full bg-gray-400" src={`http://localhost:8086${photo.profileImagePath}`} alt="" />
