...
const isCurrentUser = parseInt(userID) === userId;
  ...
  const fetchMyfriends = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.4:8080/friend-requests/${userId}/friends`, {
        method: 'GET',
        headers: {
 ...
 return (
    <div className=' flex max-w-[30rem] w-full items-center justify-center'>
      ...
      <Icon icon="fluent:edit-12-regular" data-tooltip-id="my-tooltip" data-tooltip-content="Edit privacy" className='cursor-pointer focus:outline-none text-gray-600' onClick={handleprivacyDropdown} width="1em" height="1em" />
          {dropdown &&(
...
const formattedDate = date.toLocaleDateString('en-US', options);
            ...
            <div className='flex flex-col h-60 rounded-md border border-gray-200 rounded-md w-full'>
<div className="relative">
  <img className="w-full h-24" src={`http://192.168.1.4:8086${friend.bannerImagePath}`} alt="" />
  <div className="absolute -bottom-4 justify-center w-full flex items-center">
    <img className="rounded-full w-12 h-12 border-2 border-white" alt="" src={`http://192.168.1.4:8086${friend.profileImagePath}`} />
  </div>
</div>
      <div className='flex  px-4 flex-col gap-3 py-4'>
            ...
            {parseInt(userID)===userId &&<div>
            <Icon className='cursor-pointer' onClick={()=>handleDropdown(friend.id)} icon="carbon:overflow-menu-vertical" width="1em" height="1em" />
            {activeId === friend.id && (
           ...
           {friend.id === userId ? "" : <div className='px-2 py-1 text-sm border bg-cta text-white flex justify-center rounded-md cursor-pointer hover:opacity-80'>{parseInt(userID)===userId ?(<span className=''>Message</span>) : isFriends?(<span className=''>Friends</span>):(<span className=''>Add Friend</span>)} 
            </div> }
         </div>
         </div>
                </div>
